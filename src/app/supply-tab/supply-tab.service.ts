...
export class SupplyTabService {
  ...
  private _deviceTerm = new BehaviorSubject<string>('');
  ...
  saveSupply(device: Device, stock: Stock, count: number) {
    this.deviceEventService.supplyStock(device, stock, count)
      .subscribe(res => {
        //todo : 성공 혹은 실패 안내하기
        console.log(res);
      }, error => {
  ...
  get deviceTerm() {
    return this._deviceTerm.asObservable();
  }

  setDeviceTerm(term: string) {
    this._deviceTerm.next(term);
  }
